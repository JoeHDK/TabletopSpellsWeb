          ...
          <div className="max-w-2xl mx-auto space-y-3">
            {SLOTS.map(slot => {
              ...
              return (
                <div key={slot} className="bg-gray-900 rounded-xl p-4">
                  <p className="text-sm font-semibold mb-2">{SLOT_ICON[slot]} {SLOT_LABEL[slot]}</p>
                  {slotItems.length === 0 ? (
            ...
            {items.filter(i => i.isEquipped).length === 0 && (
              <p className="text-gray-500 text-center py-10">
                ...
                <span className="text-indigo-400 text-sm">Go to Inventory and tap Equip on an item.</span>
              </p>
            )}
          </div>
        </div>
